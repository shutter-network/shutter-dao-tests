...
import {
    Azorius,
    IAzorius
} from "@fractal-framework/fractal-contracts/contracts/azorius/Azorius.sol";
...
contract AddressBook is Test {
    ...
    Azorius public azorius = Azorius(0xAA6BfA174d2f803b517026E93DBBEc1eBa26258e);
    LinearERC20Voting public strategy = LinearERC20Voting(0x4b29d8B250B8b442ECfCd3a4e3D91933d2db720F);
    ShutterToken public shu = ShutterToken(0xe485E2f1bab389C08721B291f6b59780feC83Fd7);
    VestingPoolManager public vestingPoolManager = VestingPoolManager(0xD724DBe7e230E400fe7390885e16957Ec246d716);
...
abstract contract ProposalTest is Test, AddressBook {
    ...
    function proposalFile() public virtual pure returns (string memory) {}
    ...
    function initializeVoter() public {
        ...
        uint256 amount = (n * (qDenom + qNum) * qNum + 2 * qDenom - 2) / qDenom / qDenom;
        stdstore
            .target(address(shu))
    ...
    function loadProposal(string memory filename) public view returns (Proposal memory) {
        string memory root = vm.projectRoot();
        string memory path = string.concat(root, "/encoded_proposals/", filename);
    ...
    function submitProposal(Proposal memory proposal) public returns (uint32) {
        ...
        Azorius.Transaction[] memory transactions = new Azorius.Transaction[](n);
        ...
        azorius.submitProposal(proposal.strategy, "", transactions, proposal.metadata);
        vm.stopPrank();
        vm.roll(block.number + 1);
    ...
    function executeProposal(Proposal memory proposal, uint32 proposalId) public {
        ...
        Enum.Operation[] memory operations = new Enum.Operation[](proposal.transactions.length);
